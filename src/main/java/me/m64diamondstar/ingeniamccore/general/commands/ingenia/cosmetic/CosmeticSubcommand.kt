...
class CosmeticSubcommand(private val sender: CommandSender, private val args: Array<String>): IngeniaSubcommand {
    ...
    override fun execute() {
        if (args.size <= 4) {
            sender.sendMessage(Colors.format(Messages.commandUsage("ig cosmetic <add/remove> <player> <cosmetic type> <id>")))
        }

        else if (args.size == 5) {
            val gs: CosmeticType = try {
                CosmeticType.valueOf(args[3].uppercase())
            ...
            if (args[1].equals("add", ignoreCase = true)) {
                targetPlayer.addCosmetic(gs, args[4])
                sender.sendMessage(
                    Colors.format(
                        MessageType.SUCCESS + "Successfully added cosmetic " + args[4] + " to " + target.name + "."
                    )
                )
            }

            else if (args[1].equals("remove", ignoreCase = true)) {
                targetPlayer.removeCosmetic(gs, args[4])
                sender.sendMessage(
                    Colors.format(
                        MessageType.SUCCESS + "Successfully removed cosmetic " + args[4] + " from " + target.name + "."
                    )
                )
            }
    ...
    override fun getTabCompleters(): ArrayList<String>{
        ...
        when (args.size) {
            ...
            4 -> {
                tabs.addAll(CosmeticType.values().map { it.name })
            }
            5 -> {
                if(!CosmeticType.values().map { it.name }.contains(args[3].uppercase())) tabs.add("INVALID_TYPE")
                else{
                    val cosmeticType = CosmeticType.valueOf(args[3].uppercase())
