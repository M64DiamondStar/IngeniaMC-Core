...
open class Attraction(category: String, name: String): Configuration("rides/$category", name.replace(".yml", ""), false, false) {
    ...
    fun createAttraction(type: AttractionType, world: World){
        ...
        comments.add("Please try to use in-game commands as much as possible, ")
        comments.add("but if it's really necessary you can edit this file and use")
        comments.add("/ig attraction reload ${getName()}")
        comments.add("")
        comments.add("Reminder, all the times are in ticks! 20 ticks = 1 second.")
        comments.add("-----------------------------------------")

        this.getConfig().options().setHeader(comments)
    ...
    /**
     * Spawn an EntityItemFrame with top 3 and personal ridecount of the current ride.
     */
